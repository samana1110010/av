...
from models.classifiers import VideoClassifier
...
def evaluate(model, loader, criterion, device):
    ...
    with torch.no_grad():
        for frames, _, labels, _ in loader:
...
def train_video(args):
    ...
    if args.use_retrieval_ckpt and os.path.exists("checkpoints/video_encoder_final.pt"):
        print("Loading pre-trained retrieval weights from checkpoints/video_encoder_final.pt...")
        video_encoder.load_state_dict(torch.load("checkpoints/video_encoder_final.pt", map_location=device, weights_only=True))

    model = VideoClassifier(video_encoder=video_encoder, num_classes=10, hidden_dim=args.hidden_dim, dropout=args.dropout).to(device)

    # Freeze entire encoder (backbone + unused projector)
    ...
    checkpoint_path = "checkpoints/video_classifier_best.pt"
    metrics_path = "checkpoints/video_classifier_metrics.json"
    ...
    print(f"\nStarting Video-Only Classifier Training (hidden_dim={args.hidden_dim}, dropout={args.dropout}, lr={args.lr}, weight_decay={args.weight_decay})...")

    for epoch in range(1, args.epochs + 1):
        # Optional unfreezing (disabled by default)
        if args.unfreeze_epoch > 0 and epoch == args.unfreeze_epoch:
            print(f"\n--> Epoch {epoch}: Unfreezing layer4 of ResNet50 backbone for fine-tuning!")
        ...
        for frames, _, labels, _ in train_loader:
            frames = frames.to(device)
            labels = labels.to(device)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Video-Only Classifier")
