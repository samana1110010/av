...
from models.classifiers import AudioClassifier
...
def train_audio(args):
    ...
    if args.use_retrieval_ckpt and os.path.exists("checkpoints/audio_encoder_final.pt"):
        print("Loading pre-trained retrieval weights from checkpoints/audio_encoder_final.pt...")
        audio_encoder.load_state_dict(torch.load("checkpoints/audio_encoder_final.pt", map_location=device, weights_only=True))

    model = AudioClassifier(audio_encoder=audio_encoder, num_classes=10, hidden_dim=args.hidden_dim, dropout=args.dropout).to(device)
    ...
    checkpoint_path = "checkpoints/audio_classifier_best.pt"
    metrics_path = "checkpoints/audio_classifier_metrics.json"
    ...
    print(f"\nStarting Audio-Only Classifier Training (hidden_dim={args.hidden_dim}, dropout={args.dropout}, lr={args.lr}, weight_decay={args.weight_decay})...")

    for epoch in range(1, args.epochs + 1):
        if args.unfreeze_epoch > 0 and epoch == args.unfreeze_epoch:
            print(f"\n--> Epoch {epoch}: Unfreezing layer4 of ResNet18 backbone for fine-tuning!")
        ...
        for _, mel, labels, _ in train_loader:
            mel = mel.to(device)
            labels = labels.to(device)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Audio-Only Classifier")
